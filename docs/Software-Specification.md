...
- หน้า Login และ response หลัง Logoff ตั้ง header `Cache-Control: no-store` ป้องกัน browser เก็บแคชหน้า Login เก่า (ถ้าไม่มี header นี้ browser อาจแสดงฟอร์มเปล่าที่ cached หลัง redirect จาก Logoff ทำให้ดูเหมือน pre-fill ไม่ทำงาน)
